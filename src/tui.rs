use crate::util::{BOTTOM, CLEAR, DOWN, LEFT, RIGHT, TOP, UP};
...
impl LoadingBar {
	...
	pub fn show(&self) {
		print!(
			"\r{color_side}{left}{reset}{color_empty}{empty}{reset}{color_side}{right}{reset}",
	...
	pub fn lerp(&mut self, percent: i32, unique_sides: bool) {
		let loading_length =
			(((percent as f32 / TOTAL) * self.width as f32).round() as usize).clamp(0, self.width);
		let loading = self.bar_char.to_string().repeat(loading_length);
		let padding = self
			.empty_char
			.to_string()
			.repeat(self.width - loading_length);
